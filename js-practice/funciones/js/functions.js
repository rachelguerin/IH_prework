...
function findLongestWord( array_words ){
	...
	for (word in array_words) {
		...
		if (array_words[word].length >= result) {
			result = array_words[word].length;
		}
	}
...
array_lengths.push( findLongestWord( ["Super", "Calloused", "Fragile","Midget","Cursed","with","halitosis"] ) );
array_lengths.push( findLongestWord( ["the", "quick", "brown","fox","jumped","over","the","lazy","dog"] ) );
...
console.log( calculateAverage( array_lengths ) === 6.6 && "The average of longest lengths is: " + calculateAverage( array_lengths ))
...
function charFreq( string ) {
	...
	for (s in string){
		if (freqCounter[string[s]] === undefined) {
			freqCounter[string[s]] = 1;
		} else {
			freqCounter[string[s]]++;  
		}
	}
